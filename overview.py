...
# ── AI Insight ─────────────────────────────────────────────────────────────
st.markdown("---")
st.markdown("### 🤖 AI Operational Insight")
if st.button("Generate AI insight for this market", key="ai_overview"):
    from utils.ai_engine import narrate_kpis
    with st.spinner("Analysing with AI..."):
        insight = narrate_kpis(kpis, country)
    st.markdown(insight)
else:
    st.caption("Click above for an AI-generated narrative of current KPIs.")

# ── AI Market Insight ──────────────────────────────────────────────────────
st.markdown("---")
st.markdown("### 🤖 AI Market Insight")

from utils.ai_engine import ask_ai, ai_available, active_backend, prompt_market_summary
...
if not ai_available():
    ...
    if st.button("✨ Generate AI Market Summary", key="ai_overview"):
        ai_context = {
            "country": country,
            "kpis": kpis,
            ...
            "swap_success_rate": kpis["swap_success_rate"],
            "total_customers": len(customers_f),
            "segments": customers_f["segment"].value_counts().to_dict(),
        }
        with st.spinner("Analysing market data..."):
            insight = ask_ai(prompt_market_summary(kpis, country), context=ai_context)
        st.markdown(insight)
...
scenario_choice = st.selectbox("Scenario to analyse", ["Base", "Bull", "Bear"], key="ai_fin_scenario")
if st.button("Analyse with AI", key="ai_finance", type="primary"):
    from utils.ai_engine import narrate_financial
    with st.spinner("Generating AI financial analysis..."):
        fin_insight = narrate_financial(ue, be, dcf, scenario=scenario_choice)
    st.markdown(fin_insight)
else:
    st.caption("Click above for AI interpretation of the financial model.")

# ── AI Financial Advisor ───────────────────────────────────────────────────
st.markdown("---")
st.markdown("### 🤖 AI Financial Advisor")
st.caption("Ask the AI anything about your financial model results.")

from utils.ai_engine import ask_ai, stream_ai, ai_available, active_backend, prompt_financial_qa
...
if not ai_available():
    ...
    fin_ctx = {
        ...
        "unit_economics": {
            ...
            "ebitda":                    ue["ebitda"],
            "ebitda_margin":             f"{ue['ebitda_margin_pct']:.1f}%",
            "contribution_per_swap":     ue["contribution_per_swap"],
        },
        "breakeven": {
            "swaps_per_day_needed": be["breakeven_swaps_per_day"],
            "current_swaps_per_day": swaps_day,
            "margin_of_safety":      f"{be['margin_of_safety_pct']:.1f}%",
            "payback_months":        be["payback_months"],
        },
        "valuation": {
            "npv":           dcf["npv"],
            "irr":           dcf["irr_annual_pct"],
            "total_rev_36m": dcf["total_revenue_36m"],
            "net_income_36m":dcf["total_net_income_36m"],
        },
        "monte_carlo": {
            "npv_p50":          mc["npv_p50"],
            "prob_positive_npv":f"{mc['prob_positive_npv']}%",
            "var_95":           mc["var_95"],
        },
    }
    ...
    fin_q_cols = st.columns(3)
    quick_fin_qs = [
        "Is this station economics model profitable? What needs to change?",
        "At what swap price does this become NPV-positive?",
        "What is the biggest financial risk in this model?",
    ]
    for i, q in enumerate(quick_fin_qs):
        with fin_q_cols[i]:
            if st.button(q[:40] + "...", key=f"fin_q_{i}",
                         use_container_width=True, disabled=not ai_available()):
                with st.spinner("Thinking..."):
                    ans = ask_ai(prompt_financial_qa(q), context=fin_ctx)
                st.markdown(f"**Q:** {q}")
                st.markdown(ans)

    # Free-form financial question
    st.markdown("**Ask your own financial question:**")
    user_fin_q = st.text_input(
        "Financial question",
        placeholder="e.g. How many stations do we need to reach NPV breakeven?",
        key="fin_user_q",
        label_visibility="collapsed",
    )
    if st.button("Ask AI", key="fin_ask", disabled=not (ai_available() and user_fin_q)):
        with st.chat_message("assistant", avatar="🤖"):
            placeholder = st.empty()
            full_text = ""
            for chunk in stream_ai(prompt_financial_qa(user_fin_q), context=fin_ctx):
                full_text += chunk
